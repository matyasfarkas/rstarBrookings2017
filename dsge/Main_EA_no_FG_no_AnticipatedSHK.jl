...
m = Model1011("ss23")
m1 = Model1010("ss23") # This is the same as m1011 but without anticipated shocks
...
m <= DSGE.Setting(:n_mh_simulations, 5000) # Do 5000 MH steps during estimation
...
if run_estimation
    ...
    if reoptimize(m)
           # Generalize mode_file path based on model, regime, and data_vintage
           vintage = m.settings[:data_vintage]
           mode_file = joinpath(dirname(@__FILE__), "output_data", "m1010", "ss23", "estimate", "raw","paramsmode_vint=250113.h5")
        ...
        # Use calculated ss18 mode
           mode_file = joinpath(dirname(@__FILE__), "output_data", "m1010", "ss23", "estimate", "raw","paramsmode_vint=250113.h5")
            # specify_mode!(m, mode_file)
                DSGE.update!(m, h5read(mode_file, "params")[1:88])
    ...
    if !calculate_hessian(m)
        ...
        hessian_file = joinpath(dirname(@__FILE__), "output_data", "m1011", "ss23", "estimate", "raw","hessian_vint=250116.h5")
