...
# --- 5. FINAL EXPORT ---
output_path = joinpath(saveroot, "Final Paper","Figures", "EA_rstar_pseudo_realtime_results_woFG.csv")
...
plot!(p, master_df.date, master_df[!, latest_vint]*4, # Annualize the quarterly values for better visual comparison
...
vspan!(p, [Dates.value(Date(2020, 1, 1)), Dates.value(Date(2020, 6, 30))], 
       color=:gray, alpha=0.2, label="")
vspan!(p, [Dates.value(Date(2011,9,1)), Dates.value(Date(2013,3,30))], color=:gray, alpha=0.2)
...
# 9. Save and Display
savefig(joinpath(saveroot, "Final Paper","Figures", "EA_rstar_spaghetti_chart_woFG.png"))
display(p)
...
# 8. Save
savefig(joinpath(saveroot, "Final Paper","Figures", "EA_rstar_starfish_chart_woFG.png"))
display(p_starfish)