...
if RunEstimation
    ...
    Ctr =[
           ...
           1       1       0         ;%3: Short term rate (3m TBILL)
           1       1       0         ;%4: Expected Short Rate
           1       1       1         ;%5: Long Rate (20y TBOND)
        ];
    ...
    df0tr = 100;
    %              pi^*      r^*        ts^*      cy
    SC0tr =    ([   2         1          1          ]).^2/400;%var(diff(Trend));
    S0tr =      [   2        0.5         1           ]';
    P0tr = diag([   1         1          1           ]);
    
    Psi =                  [ 2    1     1    .5      1              ];
    %                       pi   Epi   bill  Eill   bond
    S0cyc = zeros(n*p,1);
    ...
    for jm = 1:Ndraws
        ...
        if ~isnan(notrend)
            temp = S0(notrend) + randn(length(notrend),1);%(length(notrend));
            S0_new = S0; S0_new(notrend) = temp;
        ...
        if mod(jm,50)==0
            ...
            if ~isempty(notrend)
                ...
                elseif jm <10000
                    disp(['Acceptance rate of the last 1k draws: ',num2str(P_acc(jm-1000+1:jm))])
                else
                    disp(['Acceptance rate of the last 10k draws: ',num2str(P_acc(jm-1000+1:jm))])
                end;
            end;
