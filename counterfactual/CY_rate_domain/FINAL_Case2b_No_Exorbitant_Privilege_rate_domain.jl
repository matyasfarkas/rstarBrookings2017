...
mypath = @__DIR__
basepath = dirname(dirname(mypath))
...
plot!(size=(960,540))
if use_FG_in_EA 
    savefig(joinpath(figroot, "Exorbitant privilege rate_domain.pdf"))
else
    savefig(joinpath(figroot, "Exorbitant privilege without FG shocks in EA rate_domain.pdf"))
end
...
if use_FG_in_EA
    CSV.write(joinpath(figroot, "Exorbitant_privilege_rate_domain.csv"), df_out)
else
    CSV.write(joinpath(figroot, "Exorbitant_privilege_without_FG_shocks_in_EA_rate_domain.csv"), df_out)
end
...
plot(p1, p2, p3, p4, layout=(2,2), legend=false)
    ...
    savefig(joinpath(figroot, "IRF_to_permanet_liquidity_shock_scaled_rate_domain.pdf"))
...
function _policy_rate_coefficient_in_euler(m)
    sigma_c = m[Symbol(Char(0x03c3), :_c)]
    habit_term = m[:h] * exp(-m[:z_star])
    return (1.0 - habit_term) / (sigma_c * (1.0 + habit_term))
end

function _convenience_yield_rate_domain_scale(m)
    return -1.0 / _policy_rate_coefficient_in_euler(m)
end
...
cy_rate_domain_scale_US = _convenience_yield_rate_domain_scale(m1)
println("US CY rate-domain scale (-1/Xi_r): ", cy_rate_domain_scale_US)
...
cy_US_rate_domain = cy_rate_domain_scale_US .* cy_US
cy_remove_US_rate_domain = cy_rate_domain_scale_US .* cy_remove_US
cy_CF_rate_domain = cy_rate_domain_scale_US .* cy_CF
...
# -------------------------
p_cy = plot(
    dates_plot, cy_US_rate_domain,
    color = :black, lw = 2, linestyle = :solid,
    title = "Convenience yield (rate domain)",
    label = "US - Actual",
    xticks = (year_tick_dates, year_tick_labels)
)
plot!(p_cy, dates_plot, cy_remove_US_rate_domain, color = :red, lw = 2, linestyle = :dashdot, label = "US excluding matched privilege shocks")
plot!(p_cy, dates_plot, zeros(length(dates_plot)), lc = :black, lw = 2, label = "", xticks = (year_tick_dates, year_tick_labels))

plot!(
    p_cy,
    dates_plot, cy_CF_rate_domain,
    color = :blue, lw = 2,
...
if use_FG_in_EA
    savefig(joinpath(figroot, "Exorbitant privilege 3x2 rate_domain.pdf"))
    savefig(joinpath(figroot, "Exorbitant privilege 3x2 rate_domain.png"))
else
    savefig(joinpath(figroot, "Exorbitant privilege without FG shocks in EA 3x2 rate_domain.pdf"))
    savefig(joinpath(figroot, "Exorbitant privilege without FG shocks in EA 3x2 rate_domain.png"))
end
...
df_plot_3x2 = DataFrame(
    ...
    ConvenienceYieldRateDomain_Baseline = cy_US_rate_domain,
    ConvenienceYieldRateDomain_USExcludingMatchedPrivilege = cy_remove_US_rate_domain,
    ConvenienceYieldRateDomain_Counterfactual = cy_CF_rate_domain,
...
if use_FG_in_EA
    CSV.write(joinpath(figroot, "Exorbitant_privilege_3x2_rate_domain.csv"), df_plot_3x2)
else
    CSV.write(joinpath(figroot, "Exorbitant_privilege_without_FG_shocks_in_EA_3x2_rate_domain.csv"), df_plot_3x2)
end
##########################################################################################
...
## - Convenience yield shown in rate-domain APR: 4 * (-1 / Xi_r) * CY
## - LT rate, policy rate, and r* shown as APR (*4)
##########################################################################################
...
# -------------------------
# CY is shown as a policy-rate-equivalent wedge in APR:
# 4 * (-1 / Xi_r) * CY.
cy_base_ep = 4 .* cy_rate_domain_scale_US .* cy_base_raw_ep
cy_remove_cf_ep = 4 .* cy_rate_domain_scale_US .* cy_remove_raw_ep
cy_cf_ep = 4 .* cy_rate_domain_scale_US .* cy_cf_raw_ep
...
p_cy_ep = plot(
    ...
    color = :black, lw = 2, linestyle = :solid,
    title = "Convenience yield (rate domain, APR)",
    label = "US - Actual",
...
if use_FG_in_EA
    savefig(joinpath(figroot, "Exorbitant privilege 3x2 baseline_plus_delta rate_domain.pdf"))
    savefig(joinpath(figroot, "Exorbitant privilege 3x2 baseline_plus_delta rate_domain.png"))
else
    savefig(joinpath(figroot, "Exorbitant privilege without FG shocks in EA 3x2 baseline_plus_delta rate_domain.pdf"))
    savefig(joinpath(figroot, "Exorbitant privilege without FG shocks in EA 3x2 baseline_plus_delta rate_domain.png"))
end
...
p_cy_us_only = _us_black_red_panel(
    ...
    "Convenience yield (rate domain, APR)",
...
if use_FG_in_EA
    savefig(p_3x2_us_only, joinpath(figroot, "US no exorbitant privilege 3x2 baseline_plus_delta rate_domain.pdf"))
    savefig(p_3x2_us_only, joinpath(figroot, "US no exorbitant privilege 3x2 baseline_plus_delta rate_domain.png"))
else
    savefig(p_3x2_us_only, joinpath(figroot, "US no exorbitant privilege without FG shocks in EA 3x2 baseline_plus_delta rate_domain.pdf"))
    savefig(p_3x2_us_only, joinpath(figroot, "US no exorbitant privilege without FG shocks in EA 3x2 baseline_plus_delta rate_domain.png"))
end
...
df_plot_3x2_ep = DataFrame(
    ...
    ConvenienceYieldRateDomain_Baseline_APR = cy_base_ep[mask_plot_ep],
    ConvenienceYieldRateDomain_USExcludingMatchedPrivilege_APR = cy_remove_cf_ep[mask_plot_ep],
    ConvenienceYieldRateDomain_Counterfactual_APR = cy_cf_ep[mask_plot_ep],
...
if use_FG_in_EA
    CSV.write(joinpath(figroot, "Exorbitant_privilege_3x2_baseline_plus_delta_rate_domain.csv"), df_plot_3x2_ep)
else
    CSV.write(joinpath(figroot, "Exorbitant_privilege_without_FG_shocks_in_EA_3x2_baseline_plus_delta_rate_domain.csv"), df_plot_3x2_ep)
end
...
# Rate-domain APR versions. Within each model, the grouped columns still add
# cleanly because every component uses the same country-specific loading.
cy_rate_domain_scale_EA = _convenience_yield_rate_domain_scale(m)
cy_rate_domain_scale_US = _convenience_yield_rate_domain_scale(m1)
println("EA CY rate-domain scale (-1/Xi_r): ", cy_rate_domain_scale_EA)

cy_EA_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* cy_EA_full_ep
cy_US_full_apr_ep = 4 .* cy_rate_domain_scale_US .* cy_US_full_ep
ea_cy_state_total_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_cy_state_total_full_ep
us_cy_state_total_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_cy_state_total_full_ep
ea_cy_trend_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_cy_trend_full_ep
us_cy_trend_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_cy_trend_full_ep
ea_liquidity_cy_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_liquidity_cy_full_ep
ea_safety_cy_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_safety_cy_full_ep
us_liquidity_cy_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_liquidity_cy_full_ep
us_safety_cy_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_safety_cy_full_ep
ea_liquidity_state_total_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_liquidity_state_total_full_ep
us_liquidity_state_total_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_liquidity_state_total_full_ep
ea_liquidity_trend_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_liquidity_trend_full_ep
us_liquidity_trend_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_liquidity_trend_full_ep
ea_b_liqtil_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_b_liqtil_full_ep
us_b_liqtil_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_b_liqtil_full_ep
ea_b_liqp_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_b_liqp_full_ep
us_b_liqp_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_b_liqp_full_ep
ea_safety_state_total_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_safety_state_total_full_ep
us_safety_state_total_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_safety_state_total_full_ep
ea_safety_trend_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_safety_trend_full_ep
us_safety_trend_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_safety_trend_full_ep
ea_b_safetil_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_b_safetil_full_ep
us_b_safetil_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_b_safetil_full_ep
ea_b_safep_full_apr_ep = 4 .* cy_rate_domain_scale_EA .* ea_b_safep_full_ep
us_b_safep_full_apr_ep = 4 .* cy_rate_domain_scale_US .* us_b_safep_full_ep

# Export as one CSV with full samples preserved in rate-domain APR units.
df_cy_EA_ep = DataFrame(Date = dates_EA_full_ep,
                        CYRateDomainScale_EA = fill(cy_rate_domain_scale_EA, length(dates_EA_full_ep)),
                        LiquidityTransitory_EA_RateDomain_APR = ea_b_liqtil_full_apr_ep,
                        LiquidityPermanent_EA_RateDomain_APR = ea_b_liqp_full_apr_ep,
                        LiquidityStateTotal_EA_RateDomain_APR = ea_liquidity_state_total_full_apr_ep,
                        LiquidityTrend_EA_RateDomain_APR = ea_liquidity_trend_full_apr_ep,
                        LiquidityConvenienceYield_EA_RateDomain_APR = ea_liquidity_cy_full_apr_ep,
                        SafetyTransitory_EA_RateDomain_APR = ea_b_safetil_full_apr_ep,
                        SafetyPermanent_EA_RateDomain_APR = ea_b_safep_full_apr_ep,
                        SafetyStateTotal_EA_RateDomain_APR = ea_safety_state_total_full_apr_ep,
                        SafetyTrend_EA_RateDomain_APR = ea_safety_trend_full_apr_ep,
                        SafetyConvenienceYield_EA_RateDomain_APR = ea_safety_cy_full_apr_ep,
                        ConvenienceYieldStateTotal_EA_RateDomain_APR = ea_cy_state_total_full_apr_ep,
                        ConvenienceYieldTrend_EA_RateDomain_APR = ea_cy_trend_full_apr_ep,
                        ConvenienceYield_EA_RateDomain_APR = cy_EA_full_apr_ep)

df_cy_US_ep = DataFrame(Date = dates_US_ep,
                        CYRateDomainScale_US = fill(cy_rate_domain_scale_US, length(dates_US_ep)),
                        LiquidityTransitory_US_RateDomain_APR = us_b_liqtil_full_apr_ep,
                        LiquidityPermanent_US_RateDomain_APR = us_b_liqp_full_apr_ep,
                        LiquidityStateTotal_US_RateDomain_APR = us_liquidity_state_total_full_apr_ep,
                        LiquidityTrend_US_RateDomain_APR = us_liquidity_trend_full_apr_ep,
                        LiquidityConvenienceYield_US_RateDomain_APR = us_liquidity_cy_full_apr_ep,
                        SafetyTransitory_US_RateDomain_APR = us_b_safetil_full_apr_ep,
                        SafetyPermanent_US_RateDomain_APR = us_b_safep_full_apr_ep,
                        SafetyStateTotal_US_RateDomain_APR = us_safety_state_total_full_apr_ep,
                        SafetyTrend_US_RateDomain_APR = us_safety_trend_full_apr_ep,
                        SafetyConvenienceYield_US_RateDomain_APR = us_safety_cy_full_apr_ep,
                        ConvenienceYieldStateTotal_US_RateDomain_APR = us_cy_state_total_full_apr_ep,
                        ConvenienceYieldTrend_US_RateDomain_APR = us_cy_trend_full_apr_ep,
                        ConvenienceYield_US_RateDomain_APR = cy_US_full_apr_ep)

df_cy_full_ep = join(df_cy_EA_ep, df_cy_US_ep, on = :Date,  kind = :outer)

CSV.write(joinpath(figroot, "ConvenienceYield_fullsample_US_EA_rate_domain.csv"), df_cy_full_ep)
