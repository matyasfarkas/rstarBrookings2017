...
csv_path = joinpath(basepath, "Main results", "EA", "Ex_post_real_rate_gaps.csv")
EA = DataFrame(CSV.File(csv_path))

valid = findall(r -> !ismissing(r[:date]) && !ismissing(r[:actual_MP_stance]), eachrow(EA))
dates_raw = EA.date[valid]
desired_path = -collect(skipmissing(EA.actual_MP_stance[valid]))/4
...
pdf_path1 = joinpath(saveroot, "paper", "EA_What_if_real_rate_gap_change_HLW_using_policy_rate_shock.pdf")
...
pdf_path2 = joinpath(saveroot, "paper", "EA_compare_baseline_vs_HLW_change_only_ex_ante_realrate.pdf")
...
pdf_path3 = joinpath(saveroot, "paper", "EA_Paper_Compare_baseline_vs_HLW_change_only_inflation_and_output.pdf")
